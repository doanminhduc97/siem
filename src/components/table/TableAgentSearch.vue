<template>
  <v-card>
    ...
    <v-card-title>
      <v-text-field v-model="search" append-icon="mdi-magnify" :label="dataLabel" single-line hide-details>
      </v-text-field>
    </v-card-title>
    <v-data-table :headers="dataHeaders" :items="dataItems" :search="search"></v-data-table>
    <div class="download-csv">
      <button class="btn">
        <export-excel :data="dataItems" :fields="headerObj" worksheet="My Worksheet" :name="nameFileCSV" type="csv">
          <svg
            fill="#006BB4"
            width="16"
            height="16"
...
<script>
export default {
  name: 'TableAgentSearch',
  props: ['dataLabel', 'dataHeaders', 'dataItems', 'dataSearch', 'headerObj', 'nameFileCSV'],
  data() {
    return {
      search: '',
    }
